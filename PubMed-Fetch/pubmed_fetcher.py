...
# Function to fetch detailed paper information in batches
def fetch_paper_details(pubmed_ids: list, batch_size: int = 100) -> list:
    if not pubmed_ids:
        return []
    results = []
            ...
            for pid, details in data.get('result', {}).items():
                if pid != 'uids':
                    results.append({
                        ...
                        'Company': details.get('source', 'N/A')
                    })
        except requests.exceptions.RequestException as e:
            logger.error(f'Failed to fetch paper details for batch: {e}')
        time.sleep(1)  # Added delay to avoid rate limiting
    return results

# Function to save results as CSV
def save_to_csv(filename: str, papers: list) -> None:
    logger.info(f"Saving results to {filename}")
    with open(filename, mode='w', newline='', encoding='utf-8') as file:
        writer = csv.DictWriter(file, fieldnames=['PubmedID', 'Title', 'Publication Date', 'Authors', 'Company'])
        writer.writeheader()
